...
/*
	for (int nq=0; nq<NQ;nq++){
	    RG_map_fw[nq] = initial_state[nq]; //layer=0
	    RG_map_bw[nq*(L+1)] = final_state[nq]; //layer=L
	}	
*/
// FS
...
for (int l = L - 1; l >= 0; l--){ // Iterate over the layers
        ...
        switch (GT) {
			...
			case 2: {
                ...
                for (int g = 0; g < num_gates; g++, g2p0_ptr++) {
					...
					int output_RG_c;
					int output_RG_t;
                    output_RG[0] = output_RG_c;
                    output_RG[1] = output_RG_t;
	...
	for (int b=0; b<NQ; b++){
		for (int a=0; a<L-1; a++){
			...
			if (RG_map_fw[b][a]==0){
				if (RG_map_bw[b][a]==0 || RG_map_bw[b][a]==2){
					RG_map[b][a]==0;
				}
				else{
					RG_map[b][a]==3; //usar 3 como erro
				}
			}
			if (RG_map_fw[b][a]==1){
				if (RG_map_bw[b][a]==1 || RG_map_bw[b][a]==2){
					RG_map[b][a]==1;
				}
				else{
					RG_map[b][a]==3; //usar 3 como erro
